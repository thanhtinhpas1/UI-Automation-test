...
var Page = function () {
    ...
    this.findByDataTarget = async function () {
        await this.driver.wait(until.elementLocated(By.xpath("//button[@data-target='#addTeacherModal']")), 15000, 'Looking for element');
        return await this.driver.findElement(By.xpath("//button[@data-target='#addTeacherModal']"));
    }

    this.findByPlaceHolder = async function (name) {
        await this.driver.wait(until.elementLocated(By.xpath(`//input[@placeholder='${name}']`)), 15000, 'Looking for element');
        return await this.driver.findElement(By.xpath(`//input[@placeholder='${name}']`));
    };
    ...
    this.inputFindByAccept = async function() {
        await this.driver.wait(until.elementLocated(By.xpath(`//input[@accept='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet, application/vnd.ms-excel']`)), 15000, 'Looking for element');
        return await this.driver.findElement(By.xpath(`//input[@accept='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet, application/vnd.ms-excel']`));
  
    }

    this.findElementsByText = async function (value) {
        await this.driver.wait(until.elementsLocated(By.xpath(`//*[contains(text(), "${value}")]`)), 15000, 'Looking for element');
        return await this.driver.findElements(By.xpath(`//*[contains(text(), "${value}")]`));
    }
